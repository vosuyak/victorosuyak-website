...
func DisplaySuccess(w http.ResponseWriter, db interface{}, code int, response interface{}) {
	...
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(code)
	if j, err := json.Marshal(ResourceSuccess{Data: successObject}); err == nil {
		w.Write(j)
	}
}

// DisplayCustomResp - Pass the writer for the Response, msg Custom Message, pass http.Status code, Response for R
func DisplayCustomResp(w http.ResponseWriter, msg string, code int, response interface{}) {
	successObject := ServiceSuccess{
		Code:     code,
		Message:  msg,
		DB:       nil,
